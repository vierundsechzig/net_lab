...
void ip_out(buf_t *buf, uint8_t *ip, net_protocol_t protocol)
{
    ...
    {
        buf_init(&mybuf, 1500 - sizeof (ip_hdr_t));
        memcpy(mybuf.data, p, 1500 - sizeof (ip_hdr_t));
        ip_fragment_out(&mybuf, ip, protocol, id, offset, 1);
        len -= 1500 - sizeof (ip_hdr_t);
        offset += offset_step;
        p += (1500 - sizeof (ip_hdr_t));
    }
    buf_init(&mybuf, len);
    memcpy(mybuf.data, p, len);
    ip_fragment_out(&mybuf, ip, protocol, id, offset, 0);
    ++id;
}
